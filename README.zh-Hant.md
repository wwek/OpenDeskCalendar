...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/version-0.1.4--beta-0f766e" alt="version 0.1.4-beta">
...
目前倉庫已經實作 `v0.1.4-beta` 原生 Android 版本，需求來源見 `docs/open_desk_calendar_prd.md`。
...
- 可選整點 / 半點語音播報，播報前帶本地「叮咚」提示音，並支援夜間不播報和立即測試。
- 使用者主動開啟的開機自動啟動。
...
- 整點語音播報和半點語音播報預設關閉，可在設定頁分別開啟；整點播報「現在是 X 點整」，半點播報「現在是 X 點半」。
- 語音播報使用系統本地 TTS 引擎，不在應用內呼叫網路 TTS；播報前的「叮咚」提示音由應用本地產生。
- 夜間不播報預設覆蓋 22:00 至 06:00，設定頁的「立即測試」可馬上觸發提示音和語音播報。
