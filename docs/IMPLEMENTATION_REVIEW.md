...
- `Milestone 4`: settings page, theme, seconds, city, Host/Key, keep-awake, exit confirmation, night dimming, diagnostics export.
...
- `Milestone 7`: local release signing scripts, CI release build path, signed APK verification.
...
- Almanac data is generated from the MIT-licensed OPN48/cnlunar project for 2026-2099, with the old deterministic placeholder kept as fallback when a date is outside the packaged range.
...
- Indoor sensor failures keep cached data visible and write a redacted diagnostic entry.
- The 2026 holiday asset includes the expected Labor Day adjusted workday: `2026-05-09`.
- Lint reports no issues for the debug variant.
...
- Emulator smoke screenshots passed for the dark landscape dashboard and settings pages.
...
- The main dashboard no longer uses Canvas layout, but it still needs visual checks on target old phones and e-ink screens.
- The lunar/solar-term implementation should be compared with a trusted almanac dataset before release.
- Full PRD P1 scope is not complete: direct MQTT, multi-city carousel, AQI/weather warnings, holiday online update, and custom weather field mapping remain.
...
- /tmp/opendeskcalendar-prd-dashboard-final4.png
- /tmp/opendeskcalendar-prd-settings-final4.png
- /tmp/opendeskcalendar-prd-settings-device-final4.png
- /tmp/opendeskcalendar-release-smoke.png
```
